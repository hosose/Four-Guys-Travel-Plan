 ...
 *           request,HttpServletResponse response,Object hander) throws
 *           Exception{ HttpSession session = request.getSession(false);
 *           if(session==null||session.getAttribute("mvo")==null) {
 *           log.warn("##비로그인 {}##",request.getRequestURI());
 *           response.sendRedirect("/"); return false;//handler(Controller)실행 X
 *           } return true;//handler(Controller)실행 O } }
 */